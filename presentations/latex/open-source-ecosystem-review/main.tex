...
\newcommand{\card}[3]{\node[rounded corners=7pt,fill=#1,draw=#2,thick,inner sep=7pt,text width=2.75cm,align=center] {#3};}
...
\begin{frame}{Problema e pergunta de pesquisa}
  \vspace{0.40cm}
  \begin{columns}[T,onlytextwidth]
    \column{0.49\textwidth}
      \begin{tikzpicture}
        \node[rounded corners=8pt,fill=softred,draw=muted,inner sep=10pt,text width=5.2cm,align=center]
        {\Large Empresas liberam software proprietário};
      \end{tikzpicture}
    \column{0.49\textwidth}
      \begin{tikzpicture}
        \node[rounded corners=8pt,fill=softgreen,draw=utfprteal,inner sep=10pt,text width=5.2cm,align=center]
        {\Large Mas precisam formar comunidade};
      \end{tikzpicture}
  \end{columns}
  \vspace{0.55cm}
  \begin{center}
  \begin{tikzpicture}
    \node[rounded corners=6pt,fill=softblue,draw=utfprblue,thick,inner sep=12pt,text width=10.4cm,align=center]
    {\large Que critérios, processos e métricas ajudam uma empresa a transformar software fechado em ecossistema aberto sustentável?};
  \end{tikzpicture}
  \end{center}
  \source{Pergunta reconstruída a partir das cinco questões de pesquisa listadas pelos autores.}
\end{frame}
...
\begin{frame}{Como o estudo foi conduzido}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \column{0.33\textwidth}\centering\pill{softgray}{dados}\par\small produto, equipe, logs
  \end{columns}
  \source{Os autores combinam criação de modelos com aplicação em empresas, não um experimento controlado.}
\end{frame}

\begin{frame}{OSCOMM em três fases}
  \vspace{0.55cm}
  \begin{center}
  \begin{tikzpicture}[node distance=0.65cm]
    \node[rounded corners=7pt,fill=softblue,draw=utfprblue,thick,text width=3.05cm,align=center,minimum height=1.25cm] (r) {1\par Preparar\par \scriptsize prontidão};
    \node[rounded corners=7pt,fill=softyellow,draw=utfpryellow,thick,right=of r,text width=3.05cm,align=center,minimum height=1.25cm] (e) {2\par Abrir\par \scriptsize engenharia};
    \node[rounded corners=7pt,fill=softgreen,draw=utfprteal,thick,right=of e,text width=3.05cm,align=center,minimum height=1.25cm] (w) {3\par Observar\par \scriptsize comunidade};
    \draw[-{Stealth[length=3mm]},line width=1.1pt] (r)--(e);
    \draw[-{Stealth[length=3mm]},line width=1.1pt] (e)--(w);
  \end{tikzpicture}
  \end{center}
  \vspace{0.40cm}
  \begin{center}\large O modelo transforma abertura em um processo de decisão.\end{center}
  \source{Framework OSCOMM: Release Readiness Rating, Open Source Engineering e Community Watchdog.}
\end{frame}
...
\begin{frame}{Gurux: o caso que torna o custo visível}
  ...
  \end{columns}
  \vspace{0.42cm}
  \begin{center}\large O esforço foi subestimado; documentação e reescrita levaram quase o dobro do planejado.\end{center}
  \source{Caso Gurux: abertura em 2009, refatoração, documentação, fórum, rastreador de erros e monitoramento da comunidade.}
\end{frame}
...
\begin{frame}{O que o artigo demonstra e o que não demonstra}
  ...
  \end{columns}
  \source{Os próprios autores reconhecem limites de validade: confidencialidade, poucos casos, granularidade desigual e participação ativa dos pesquisadores.}
\end{frame}
...
\begin{frame}{Confronto com a prática: LibreSign}
  ...
  \source{A ponte com LibreSign/LibreCode ajuda a formular hipóteses futuras: qualidade, entrada de contribuidores, governança e sustentabilidade.}
\end{frame}
...
\begin{frame}{Síntese crítica}
  ...
  \source{Conclusão: OSCOMM documenta elementos necessários, mas os autores afirmam que mais casos reais são necessários para consolidar a área.}
\end{frame}
