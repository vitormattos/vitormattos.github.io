...
final class TalkTopics
{
    ...
    public static function resolve(object $talk): array
    {
        ...
        if (is_array($presentation)) {
            ...
            if ($metadataPath !== '') self::collectFromMetadata($topics, $metadataPath);
        }
    ...
    public static function mergeCatalog(iterable $preferred, iterable $fallback): array
    {
        ...
        foreach ([$preferred, $fallback] as $collection) {
            foreach ($collection as $talk) {
                ...
                $seen[$identity] = true; $items[] = $talk;
            }
        }
        usort($items, static fn(object $left, object $right): int => (int) ($right->date ?? 0) <=> (int) ($left->date ?? 0));
        return $items;
    }
    ...
    public static function taxonomy(iterable $talks): array
    {
        ...
        foreach ($talks as $talk) {
            ...
            foreach (self::resolve($talk) as $key => $label) {
                if (!isset($taxonomy[$key])) $taxonomy[$key] = ['label' => $label, 'count' => 0];
                elseif (self::preferLabel($label, $taxonomy[$key]['label'])) $taxonomy[$key]['label'] = $label;
                ++$taxonomy[$key]['count'];
            }
        }

        $taxonomy = array_filter($taxonomy, static fn(array $topic): bool => $topic['count'] >= self::CATALOG_MINIMUM_OCCURRENCES);
    ...
    private static function identity(object $talk): string
    {
        ...
        if (is_array($presentation)) {
            ...
            if ($url !== '') return 'url:' . rtrim($url, '/');
        }
        if ($talk->slidesId ?? false) return 'slides:' . (string) $talk->slidesId;
        return 'slug:' . (string) ($talk->slug ?? $talk->title ?? spl_object_id($talk));
    }

    private static function collect(array &$topics, mixed $value): void
    {
        if (is_array($value)) { foreach ($value as $item) self::collect($topics, $item); return; }
        ...
        if (!isset($topics[$key]) || self::preferLabel($label, $topics[$key])) $topics[$key] = $label;
    }
    ...
    private static function collectFromMetadata(array &$topics, string $metadataPath): void
    {
        ...
        try { $metadata = json_decode((string) file_get_contents($path), true, flags: JSON_THROW_ON_ERROR); } catch (\Throwable) { return; }
        if (is_array($metadata) && array_key_exists('tags', $metadata)) self::collect($topics, $metadata['tags']);
    }
