...
final class PdfExportPolicy
{
    ...
    {
        $files = [__FILE__, __DIR__ . '/SlidesDeckPolicy.php', dirname(__DIR__, 2) . '/scripts/generate-slides-pdfs.php'];
    ...
    public static function cachePath(string $cacheDirectory, string $deckDirectory, array $metadata, ?string $generatorFingerprint = null): string
    {
        $deckId = preg_replace('/[^a-zA-Z0-9._-]/', '-', (string) ($metadata['id'] ?? 'deck'));
        return rtrim($cacheDirectory, '/') . '/' . $deckId . '-' . self::sourceFingerprint($deckDirectory, $generatorFingerprint) . '.pdf';
    }

    public static function exportManifest(string $deckDirectory, array $metadata, string $pdfPath, ?string $generatorFingerprint = null): array
    {
        $generator = $generatorFingerprint ?? self::generatorFingerprint();
        return [
            ...
            'generator' => ['sha256' => $generator, 'decktape' => self::DECKTAPE_VERSION],
            'pdf' => [
    ...
    public static function manifestMatches(string $manifestPath, string $deckDirectory, array $metadata, ?string $generatorFingerprint = null): bool
    {
    ...
    public static function deckTapeArguments(array $metadata, string $output): array
    {
        ...
        return [
            'npx', '--yes', 'decktape@' . self::DECKTAPE_VERSION, 'reveal',
            '--size', SlidesDeckPolicy::viewport($metadata),
            '--load-pause', '3000', '--pause', '250',
