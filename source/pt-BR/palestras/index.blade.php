        ...
        <aside class="talks-sidebar" aria-label="Palestrante e tópicos">
            ...
            @if ($topics !== [])
                ...
                <nav id="talk-topic-filter" class="talk-tag-filter" data-talk-tag-filter
                    aria-label="Filtrar palestras por tópico">
                    <p class="talk-tag-filter__label">Tópicos</p>
                    <a class="talk-tag talk-tag--filter" href="{{ $page->baseUrl }}/pt-BR/palestras/" data-talk-tag=""
                        aria-current="true"><span>Todas</span><span>{{ count($talkItems) }}</span></a>
                    @foreach ($topics as $topic => $data)
                        <a class="talk-tag talk-tag--filter"
                            href="{{ $page->baseUrl }}/pt-BR/palestras/?tag={{ rawurlencode($topic) }}"
                ...
                <div class="gallery-switcher" data-gallery-switcher>
                    ...
                    <div class="gallery-switcher__menu" data-gallery-menu hidden role="group"
                        ...
                        <button type="button" data-gallery-view="grid" aria-pressed="true">
                            <span><strong>Grade</strong><small>Imagens grandes e informações concisas</small></span><span
