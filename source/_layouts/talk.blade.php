...
@php
    ...
    $resourceHref = static fn(array $item): string => \App\Presentations\TalkMetadata::publicHref(
        (string) ($item['href'] ?? ''),
        (string) $page->baseUrl,
    );
@endphp
@push('head')
    ...
    @if ($presentation['localCss'] ?? false)
        <link rel="stylesheet" href="{{ $page->baseUrl }}{{ $presentation['localCss'] }}">
    @endif
@endpush
@if ($needsReveal)
    @push('scripts')
        ...
        @if (($presentation['slideCount'] ?? 0) || (($presentation['width'] ?? 0) && ($presentation['height'] ?? 0)))
            <dl class="presentation-metadata">
                ...
                @if (($presentation['width'] ?? 0) && ($presentation['height'] ?? 0))
                    <div>
                        <dt>{{ $isEnglish ? 'Canvas' : 'Tela' }}</dt>
                        <dd>{{ $presentation['width'] }} × {{ $presentation['height'] }}</dd>
                    </div>
                @endif
            </dl>
        @endif
                ...
                @if ($appearances !== [])
                    ...
                    <ul class="talk-history__list">
                        @foreach ($appearances as $appearance)
                            <li>
                                <div>
                                    @if ($appearance['href'] ?? false)
                                        <a href="{{ $resourceHref($appearance) }}" target="_blank"
                                            rel="external noopener noreferrer">{{ $appearance['event'] ?? ($isEnglish ? 'Event' : 'Evento') }}</a>@else{{ $appearance['event'] ?? ($isEnglish ? 'Event' : 'Evento') }}
                                    @endif
                                    @if ($appearance['date'] ?? false)
                                        · <time
                                            datetime="{{ $appearance['date'] }}">{{ $isEnglish ? $appearance['date'] : date('d/m/Y', strtotime($appearance['date'])) }}</time>
                                    @endif
                                    @if ($appearance['venue'] ?? false)
                                        · {{ $appearance['venue'] }}
                                    @endif
                                    @if ($appearance['city'] ?? false)
                                        · {{ $appearance['city'] }}
                                    @endif
                                    @if ($appearance['country'] ?? false)
                                        · {{ $appearance['country'] }}
                                    @endif
                                    @if ($appearance['mode'] ?? false)
                                        · {{ $appearance['mode'] }}
                                    @endif
                                </div>
                                @if (($appearance['resources'] ?? []) !== [])
                                    <ul class="talk-history__links">
                                        @foreach ((array) $appearance['resources'] as $resource)
                                            <li><a href="{{ $resourceHref((array) $resource) }}" target="_blank"
                                                    rel="external noopener noreferrer">{{ $resource['label'] ?? ucfirst((string) ($resource['type'] ?? ($isEnglish ? 'resource' : 'recurso'))) }}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
                ...
                @if ($sources !== [])
                    ...
                    <ul class="talk-history__links">
                        @foreach ($sources as $source)
                            <li><a href="{{ $resourceHref((array) $source) }}" target="_blank"
                                    rel="external noopener noreferrer">{{ $source['label'] ?? strtoupper((string) ($source['type'] ?? ($isEnglish ? 'Source' : 'Fonte'))) }}</a>
                            </li>
                        @endforeach
                    </ul>
                @endif
