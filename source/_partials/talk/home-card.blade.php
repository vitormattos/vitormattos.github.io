...
@php
    ...
    if ($talk->slidesId ?? false) {
        ...
        if ($localThumbnails !== []) {
            ...
            if (is_array($imageSize)) { $thumbnailWidth = (int) $imageSize[0]; $thumbnailHeight = (int) $imageSize[1]; }
        }
    }
@endphp
<article class="home-talk-card">
    ...
    <div class="home-talk-card__body">
        <div class="home-talk-card__meta">@if ($talk->date ?? false)<time datetime="{{ date('Y-m-d', $talk->date) }}">{{ date($isEnglish ? 'M d, Y' : 'd/m/Y', $talk->date) }}</time>@endif @if ($presentation['slideCount'] ?? 0)<span>{{ $presentation['slideCount'] }} slides</span>@endif</div>
