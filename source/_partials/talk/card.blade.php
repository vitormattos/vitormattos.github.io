...
<article class="talk-card">
    <div class="talk-card__preview" aria-hidden="true">
        ...
        @elseif ($type === 'reveal' && $sourcePath)
            <div class="presentation-thumbnail"><div class="reveal js-reveal-deck" id="{{ $thumbnailId }}" data-presentation-mode="thumbnail"><div class="slides"><section data-markdown="{{ $page->baseUrl }}{{ $sourcePath }}" data-separator="^\r?\n---\r?\n$" data-separator-vertical="^\r?\n--\r?\n$" data-separator-notes="^Notes?:"></section></div></div></div>
        @elseif (in_array($type, ['slides.com', 'iframe'], true) && ($presentation['embed'] ?? false))
            <iframe src="{{ $presentation['embed'] }}" title="" loading="lazy" tabindex="-1"></iframe>
        @else
            <div class="presentation-fallback"><strong>{{ $talk->title }}</strong></div>
        @endif
    </div>
    <div class="talk-card__body">
        @if ($talk->date ?? false)<p class="meta"><time datetime="{{ date('Y-m-d', $talk->date) }}">{{ date($isEnglish ? 'Y-m-d' : 'd/m/Y', $talk->date) }}</time></p>@endif
        <h2 class="talk-card__title"><a href="{{ $talk->getUrl() }}">{{ $talk->title }}</a></h2>
        <p>{{ $talk->description }}</p>
        @if (($presentation['slideCount'] ?? 0) || ($presentation['language'] ?? false))
            <div class="talk-card__metadata">
                @if ($presentation['slideCount'] ?? 0)<span>{{ $presentation['slideCount'] }} {{ $isEnglish ? 'slides' : 'slides' }}</span>@endif
                @if ($presentation['language'] ?? false)<span>{{ $presentation['language'] }}</span>@endif
                @if ($presentation['themeColor'] ?? false)<span>{{ $presentation['themeColor'] }}</span>@endif
            </div>
        @endif
