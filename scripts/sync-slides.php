...
for ($page = 2; $page <= $pages; ++$page) {
    $next = request("/v1/decks?per_page=100&page={$page}");
    array_push($summaries, ...($next['data'] ?? []));
}
...
foreach ($summaries as $summary) {
    ...
    $expected[$managedName] = true;
    $locale = ($detail['language'] ?? '') === 'pt-BR' || str_starts_with((string) ($detail['language'] ?? ''), 'pt') ? 'pt-BR' : 'en';
    ...
    $meta = [
        'id' => $detail['id'], 'slug' => $slug, 'visibility' => $detail['visibility'],
        'slide_count' => $detail['slide_count'] ?? null, 'width' => $detail['width'] ?? null,
        'height' => $detail['height'] ?? null, 'margin' => $detail['margin'] ?? null,
        'transition' => $detail['transition'] ?? null, 'background_transition' => $detail['background_transition'] ?? null,
        'rtl' => $detail['rtl'] ?? false, 'loop' => $detail['loop'] ?? false,
        'theme_font' => $detail['theme_font'] ?? null, 'theme_color' => $detail['theme_color'] ?? null,
        'language' => $detail['language'] ?? null, 'created_at' => $detail['created_at'] ?? null,
        'updated_at' => $detail['updated_at'] ?? null, 'urls' => $detail['urls'] ?? [],
    ];
    ...
    writeIfChanged("{$collection}/{$managedName}.md", $frontMatter);
}
...
fwrite(STDOUT, 'Synchronized ' . count($expected) . " public Slides.com decks.\n");
