...
// Based on DeckTape's Reveal plugin, with Slides.com embed cleanup for the POC.
...
export const create = (page, opts) => new RevealPlugin(page, opts);
...
class RevealPlugin {
  ...
  getName() { return 'Reveal JS (Slides.com POC)'; }

  isActive() {
    return this.page.evaluate(() => {
      if (typeof Reveal === 'undefined') return false;
      if (typeof Jupyter !== 'undefined') return false;
  ...
  async configure() {
    await this.page.evaluate(config => {
      ...
      style.dataset.decktapeSlidesPoc = 'true';
      style.textContent = `
  ...
  slideCount() {
    return this.page.evaluate(() => typeof Reveal.getTotalSlides === 'function' ? Reveal.getTotalSlides() : undefined);
  }
  ...
  nextSlide() { return this.page.evaluate(() => Reveal.next()); }

  currentSlideIndex() {
    return this.page.evaluate(() => {
      const indices = Reveal.getIndices();
      const id = Reveal.getCurrentSlide().getAttribute('id');
      return typeof id === 'string' && id.length ? '/' + id : '/' + indices.h + (indices.v > 0 ? '/' + indices.v : '');
    });
  }
}
